...
class WGAN_GP(AbstractGANModel):
    ...
    def generator(self, z, reuse=False, name='generator'):
        with tf.variable_scope(name, reuse=reuse):
            ...
            seq.add_layer(conv2d_transpose, [self.batch_size, 8, 8, 256], CONV_FILTER_7722)
            seq.add_layer(bn)
            seq.add_layer(relu)

            seq.add_layer(conv2d_transpose, [self.batch_size, 16, 16, 128], CONV_FILTER_7722)
            seq.add_layer(bn)
            seq.add_layer(relu)

            seq.add_layer(conv2d_transpose, [self.batch_size, 32, 32, self.input_c], CONV_FILTER_7722)
