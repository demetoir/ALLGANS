...
class AAE(AbstractModel):
    ...
    def discriminator_gauss(self, z, reuse=False, name='discriminator_gauss'):
        with tf.variable_scope(name, reuse=reuse):
            layer = Stacker(z)
    ...
    def discriminator_cate(self, z, reuse=False, name='discriminator_cate'):
        with tf.variable_scope(name, reuse=reuse):
            layer = Stacker(z)
            layer.linear_block(256, relu)
            layer.linear_block(256, relu)
    ...
    def load_main_tensor_graph(self):
        self.Xs_real = tf.placeholder(tf.float32, self.Xs_shape, name='Xs_real')
        self.Ys_real = tf.placeholder(tf.float32, self.Ys_shape, name='Ys_real')
        self.zs_real = tf.placeholder(tf.float32, self.zs_shape, name='zs_real')
        ...
        if not is_vector(self.X_shape):
            self.Xs_flatten = flatten(self.Xs_real)
            Xs = self.Xs_flatten
            self.X_flatten_size = Xs.shape[1]

        self.zs_gen, self.Ys_gen, self.hs = self.encoder(Xs)
        self.code = self.zs_gen
        self.Xs_gen = self.decoder(self.zs_gen, self.Ys_gen)

        self.D_gauss_real = self.discriminator_gauss(self.zs_real)
        self.D_gauss_gen = self.discriminator_gauss(self.zs_gen, reuse=True)

        self.D_cate_real = self.discriminator_cate(self.Ys_real)
        ...
        self.predict = tf.equal(tf.arg_max(self.hs, 1), tf.arg_max(self.Ys_real, 1), name='predict')
        self.acc = tf.reduce_mean(tf.cast(self.predict, tf.float32), name='acc')

    def load_loss_function(self):
        with tf.variable_scope('loss'):
            # AE loss
            X = flatten(self.Xs_real)
            X_gen = flatten(self.Xs_gen)
            self.loss_AE = tf.squared_difference(X, X_gen, name="loss_AE")
            ...
            # classifier phase
            self.loss_clf = tf.nn.softmax_cross_entropy_with_logits_v2(labels=self.Ys_real,
                                                                       logits=self.hs,
                                                                       name='loss_clf')
    ...
    def get_tf_values(self, sess, fetches, Xs=None, Ys=None, zs_real=None):
        return sess.run(
            fetches=fetches,
            feed_dict={
                self.Xs_real: Xs,
