...
class InstanceManager:
    ...
    @log_exception
    def train_instance(self, dataset=None, epoch=None, check_point_interval=None,
                       is_restore=False):
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(epoch):
                for _ in range(iter_per_epoch):
                    iter_num += 1
                    self.model.train(sess=sess, iter_num=iter_num, dataset=dataset)
    ...
    @log_exception
    def sampling_instance(self, is_restore=False):
    ...
    def open_subprocess(self, args, process_name):
        ...
        self.log("open subprocess pid:%s, cmd='%s'" % (pid, str_args))

    def close_subprocess(self, process_name):
        if process_name in self.sub_process:
            self.log("kill subprocess pid:%s, '%s'" % (self.sub_process[process_name].pid, process_name))
